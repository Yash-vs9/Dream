...
const callGeminiText = async (apiKey, history, message) => {
  ...
  const prompt = {
    ...
    generationConfig: {
      maxOutputTokens: 100,
      temperature: 1.2, // Higher temperature for more creative/dreamy output
    }
  };
...
const callGoogleTTS = async (apiKey, text) => {
  ...
  const payload = {
    input: { text: text },
    // Using a soft, breathy voice if available, or standard female
    voice: { languageCode: "en-US", name: "en-US-Journey-F" }, 
    audioConfig: { audioEncoding: "MP3", speakingRate: 0.95, pitch: 2.0 } // Higher pitch for fairy voice
  };
...
const speakNativeBrowser = (text, onEnd) => {
  ...
  utterance.rate = 0.9;
  utterance.pitch = 1.4; // Fairy-like pitch
...
function VRMModel({ url, onLoaded, setVrmRef, isSpeaking }) {
  ...
  useEffect(() => {
    ...
    // Initial Pose: Arms slightly out (A-Pose)
  ...
  useFrame(({ clock }) => {
    ...
    // 2. Fairy Hand Gestures (Only when speaking)
    const lArm = vrm.humanoid.getRawBoneNode("leftUpperArm");
    const rArm = vrm.humanoid.getRawBoneNode("rightUpperArm");
    ...
    if (isSpeaking) {
      // Gentle waving/casting gestures
...
function AnimeGuide({ vrm, analyser }) {
  ...
  useFrame(() => {
    ...
    if (analyser && vrm.expressionManager) {
      ...
      // Increased sensitivity for whispery fairy voice
      ...
      // Always look a bit happy/mystical
...
export default function DreamVRMApp() {
  ...
  if (!mounted) return <div className="bg-black h-screen w-full flex items-center justify-center text-white">Summoning Dream...</div>;

  return (
    <div className="relative w-full h-screen bg-black text-white overflow-hidden" onClick={() => !audioReady && initAudio()}>
      <Canvas camera={{ position: [0, 1.25, 3.5], fov: 30 }}>
        <ambientLight intensity={0.6} />
        <directionalLight intensity={1.5} position={[2, 2, 5]} color="#d8b4fe" /> {/* Purple Light */}
        <pointLight position={[-2, 1, 2]} intensity={1.5} color="#818cf8" /> {/* Blue Light */}
      ...
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="absolute bottom-10 left-1/2 -translate-x-1/2 z-50 w-96 backdrop-blur-md bg-white/5 border border-purple-500/30 rounded-3xl p-6 shadow-2xl shadow-purple-900/20">
        ...
        {!apiKey ? (
           <div className="flex flex-col gap-2">
               <input 
                 ...
                 className="w-full bg-transparent border-b border-white/20 text-white pb-2 text-center focus:outline-none" 
                 onChange={e => {
           ...
           <button onClick={initAudio} className="w-full py-3 bg-purple-600 hover:bg-purple-500 transition rounded-full font-bold flex items-center justify-center gap-2">
             <Play size={16}/> Awaken Lumina
           </button>
        ) : (
           <div className="flex gap-3">
             <button onClick={startListening} className="flex-1 bg-gradient-to-r from-purple-600 to-indigo-600 rounded-full h-12 flex items-center justify-center hover:scale-105 transition shadow-lg shadow-purple-500/20">
                <Mic className="text-white" />
             </button>
             <button onClick={() => handleSpeak("Tell me a secret about the stars.")} className="px-6 rounded-full border border-white/10 hover:bg-white/10 text-sm">
                Ask
             </button>
           </div>
        )}
